...
#define CHECK(status)                                                                                                  \
    ...
    {                                                                                                                  \
        ...
        if (ret != 0)                                                                                                  \
        {                                                                                                              \
            inference::gLogError << "Cuda failure: " << ret << std::endl;                                                 \
            abort();                                                                                                   \
        }                                                                                                              \
    } while (0)
...
#define CHECK_RETURN_W_MSG(status, val, errMsg)                                                                        \
    ...
    {                                                                                                                  \
        if (!(status))                                                                                                 \
        {                                                                                                              \
            inference::gLogError << errMsg << " Error in " << __FILE__ << ", function " << FN_NAME << "(), line "         \
                              << __LINE__ << std::endl;                                                                \
            return val;                                                                                                \
        }                                                                                                              \
    } while (0)
...
#define ASSERT(condition)                                                                                              \
    ...
    {                                                                                                                  \
        if (!(condition))                                                                                              \
        {                                                                                                              \
            inference::gLogError << "Assertion failure: " << #condition << std::endl;                                     \
            abort();                                                                                                   \
        }                                                                                                              \
    } while (0)
...
{
    const auto inds = inferenceCommon::argMagnitudeSort(output.cbegin(), output.cend());
...
{
    std::vector<size_t> indices = inferenceCommon::argMagnitudeSort(v.cbegin(), v.cend());
    indices.resize(k);
    return indices;
}
...
template <int C, int H, int W>
void readPPMFile(const std::string& filename, inferenceCommon::PPM<C, H, W>& ppm)
{
    ...
    //std::cout << "PPM: " << ppm.magic << " " << ppm.w << " " << ppm.h << " " << ppm.max << " " << std::endl;
    infile.seekg(1, infile.cur);
    infile.read(reinterpret_cast<char*>(ppm.buffer), ppm.w * ppm.h * C);
    infile.close();
}
...
inline void loadLibrary(const std::string& path)
{
    ...
    {
#ifdef _MSC_VER
        inference::gLogError << "Could not load plugin library: " << path << std::endl;
#else
        inference::gLogError << "Could not load plugin library: " << path << ", due to: " << dlerror() << std::endl;
#endif
    }
}
